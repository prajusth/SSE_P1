...
def plot_violins(df, metric, ylabel):
    ...
    for level in LEVEL_ORDER:
        for ft in sorted(df["file_type"].unique()):
            ...
            save_plot(fig, f"violin_{level}_{ft}.png")
...
def plot_energy_per_mb(df):
    ...
    for level in LEVEL_ORDER:
        ...
        save_plot(fig, f"energy_per_mb_{level}.png")
...
def plot_ratio_vs_energy(df, metric):
    ...
    plt.tight_layout()
    save_plot(fig, "ratio_vs_energy.png")
...
def plot_scaling(df):
    if "energy_per_mb" not in df.columns or df["energy_per_mb"].notna().sum() < 10:
        return
    print("  scaling...")
    mb_map = {"small": 10, "medium": 100, "large": 500}
    sizes = _size_order(df)
    if len(sizes) < 2:
        return
...
def plot_edp(df):
    ...
    for level in LEVEL_ORDER:
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.95])
        save_plot(fig, f"edp_{level}.png")
...
def plot_heatmap(df, metric):
    ...
    for level in LEVEL_ORDER:
        ...
        plt.tight_layout()
        save_plot(fig, f"heatmap_{level}.png")
...
def plot_level_heatmap(df, metric):
    ...
    plt.tight_layout()
    save_plot(fig, "level_comparison_heatmap.png")
